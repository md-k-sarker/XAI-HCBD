package edu.wright.dase.explanation.dllearner;
...
public class MapADEToSumo extends ModifyOntology {
    ...
    /* Folder name is the key and set< of .txt file> (annotations files) are the value.*/
    ...
    /**
     * @param imgContains the imgContains to set
     */
    ...
    /**
     * @param file_mapping: the file_browser_mapping to set
     */
	 ...
	 */
	//@formatter:on
    public List<OWLOntologyChange> extractTagText(Path path, Integer individualNo) {
    ...
    public void createObjectProperty() {
        ...
        logger.info("imgContains property name: " + imgContains.getIRI());
    }
    ...
    public void createClassIndiHierarchy() {
        ...
        if (concept_indi_mapping != null && !concept_indi_mapping.isEmpty()) {
            for (String a_concept_name : concept_indi_mapping.keySet()) {
                ...
                // check whether the concept already exist
    ...
    public void processDirs(Path path) {
        ...
        if (pathParts.length == 1) {
            ...
            if (pathParts[1].equalsIgnoreCase("indoor") || pathParts[1].equalsIgnoreCase("outdoor")) {
                // to check whether the concept already exist
    ...
    public static void doOps() {
        try {
            ...
            String filePath = "/Users/sarker/Workspaces/ProjectHCBD/datas/ade20k/validation/z/zoo/ADE_val_00000934_atr.txt";

            mapADEToSumo.init();

            logger.info("Original/Base Ontology loading...............");
            mapADEToSumo.loadOntology();
            logger.info("Original/Base Ontology loading finished.");

            mapADEToSumo.createObjectProperty();

            logger.info("Extracting tag texts...............");
            mapADEToSumo.extractTagText(Paths.get(filePath), 0);
            logger.info("Extracting tag texts finished.");

            logger.info("Writing changes to ontology...............");
            mapADEToSumo.writeChanges();
            logger.info("Writing changes to ontology finished.");
            ...
            logger.error("Fatal Error: " + Utility.getStackTraceAsString(ex));
        }
    }
